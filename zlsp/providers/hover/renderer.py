...
from zlsp.lsp_types import SemanticToken, Position
from zlsp.token_types import TokenType
from zlsp.parser.basic.type_hints import TYPE_HINT_PATTERN
from ..shared.documentation_registry import DocumentationRegistry
...
class HoverRenderer:
    ...
    def render(content: str, line: int, character: int, tokens: list[SemanticToken]) -> Optional[str]:
        ...
        logger.info("🔍 HoverRenderer.render: line=%s, char=%s", line, character)
        logger.info("   Total tokens received: %s", len(tokens))

        # Find token at position
        target_pos = Position(line=line, character=character)
        token = HoverRenderer._find_token_at_position(tokens, target_pos)

        logger.info("   Token: %s", token.token_type if token else 'None')
    ...
    def _find_token_at_position(tokens: list[SemanticToken], pos: Position) -> Optional[SemanticToken]:
        ...
        logger.info("   Found %s tokens on line %s", len(nearby), pos.line)
        for t in nearby:
            logger.info(
                "     Token: type=%s, char=%s-%s, contains=%s",
                t.token_type, t.start_char, t.start_char+t.length, t.range.contains(pos)
            )
    ...
    @staticmethod
    def _render_key(_token: SemanticToken, content: str, line: int) -> Optional[str]:
        ...
        try:
            ...
            if HoverRenderer._emoji_descriptions is None:
                ...
                try:
                    ...
                    from zSys.accessibility import get_emoji_descriptions  # type: ignore[import-not-found]
