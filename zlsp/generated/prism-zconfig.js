...
Prism.languages.zconfig = Prism.languages.extend('zolo', {
    ...
    'zmachine-locked-section': {
        pattern: /(?<=\n)[ \t]{1}(?:cpu|display|gpu|launch_commands|machine_identity|memory|network|python_runtime|storage|user_paths)(?=\s*:)/m,
        alias: 'constant',
        lookbehind: true,
    },
    'zmachine-editable-section': {
        pattern: /(?<=\n)[ \t]{1}(?:custom|time_date_formatting|user_preferences)(?=\s*:)/m,
        alias: 'variable',
        lookbehind: true,
    },
    'zconfig-property': {
