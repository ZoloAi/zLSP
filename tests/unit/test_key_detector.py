...
from zlsp.parser.zvaf.file_type_detector import FileTypeDetector
...
def make_emitter(filename: str) -> TokenEmitter:
    """
    Build a TokenEmitter with explicit file-type flags.

    TokenEmitter no longer auto-detects file type from `filename`; this mirrors
    how zlsp/parser/parser.py constructs it via FileTypeDetector.
    """
    detector = FileTypeDetector(filename)
    return TokenEmitter(
        "",
        filename=filename,
        is_zui_file=detector.is_zui() or detector.is_zraven(),
        is_zenv_file=detector.is_zenv(),
        is_zspark_file=detector.is_zspark(),
        is_zconfig_file=detector.is_zconfig(),
        is_zschema_file=detector.is_zschema(),
        zui_component_name=detector.component_name if (detector.is_zui() or detector.is_zraven()) else None,
        zspark_component_name=detector.component_name if detector.is_zspark() else None,
        zconfig_component_name=detector.component_name if detector.is_zconfig() else None,
    )
...
class TestRootKeyDetection:
    ...
    def test_zmeta_in_zui_file(self):
        ...
        emitter = make_emitter("zUI.zVaF.zolo")
    ...
    def test_component_name_in_zui_file(self):
        ...
        emitter = make_emitter("zUI.MyComponent.zolo")
    ...
    def test_zmeta_in_zschema_file(self):
        ...
        emitter = make_emitter("zSchema.users.zolo")
        token_type = KeyDetector.detect_root_key('zMeta', emitter, 0)
        assert token_type == TokenType.ZMETA_KEY
    
    def test_zspark_in_zspark_file(self):
        """Test zSpark root key in zSpark files"""
        # zSpark highlighting is parked (being rebuilt from scratch),
        # so zSpark: falls through to the default ROOT_KEY.
        emitter = make_emitter("zSpark.example.zolo")
    ...
    def test_zenv_config_keys(self):
        ...
        emitter = make_emitter("zEnv.development.zolo")
    ...
    def test_zconfig_component_name(self):
        ...
        emitter = make_emitter("zConfig.machine.zolo")
    ...
    def test_regular_root_key(self):
        ...
        emitter = make_emitter("basic.zolo")
...
class TestNestedKeyDetection:
    ...
    def test_gate_knob_keys(self):
        """Gate knobs inside zGate/zRBAC blocks (spec + legacy navbar knobs)"""
        from zlsp.token_registry import BLOCK_ZGATE

        # Spec knobs inside a zGate block
        emitter = make_emitter("zUI.zVaF.zolo")
        emitter.enter_block(BLOCK_ZGATE, 2, 0)
        for key in ['authed', 'role', 'require', 'zAll', 'zAny', 'zNot']:
            token_type = KeyDetector.detect_nested_key(key, emitter, 4)
            assert token_type == TokenType.ZRBAC_OPTION_KEY

        # Legacy navbar knobs inside a deprecated zRBAC block
        emitter = make_emitter("zEnv.development.zolo")
        emitter.enter_block(BLOCK_ZRBAC, 2, 0)
        for key in ['authenticated', 'require_role', 'zGuest']:
            token_type = KeyDetector.detect_nested_key(key, emitter, 4)
            assert token_type == TokenType.ZRBAC_OPTION_KEY

    def test_zgate_key_detection(self):
        """zGate (and deprecated zRBAC) get the gate token in zUI/zEnv files"""
        for fname in ("zUI.zVaF.zolo", "zEnv.development.zolo"):
            emitter = make_emitter(fname)
            assert KeyDetector.detect_nested_key('zGate', emitter, 2) == TokenType.ZRBAC_KEY
            assert KeyDetector.detect_nested_key('zRBAC', emitter, 2) == TokenType.ZRBAC_KEY

    def test_zmodal_key_detection(self):
        """zModal navigation verb is a UI element key in zUI files"""
        emitter = make_emitter("zUI.zVaF.zolo")
        assert KeyDetector.detect_nested_key('zModal', emitter, 2) == TokenType.UI_ELEMENT_KEY
...
class TestUIElementKeys:
    ...
    def test_plural_shorthand_keys(self):
        ...
        emitter = make_emitter("zUI.zVaF.zolo")
        # Plural shorthands are not in UI_ELEMENT_MAPPING, so KeyDetector
        # treats them as regular nested keys (handled elsewhere via
        # token_registry.PLURAL_SHORTHAND_KEYS).
        for key in ['zURLs', 'zTexts', 'zImages', 'zMDs', 'zH1s', 'zH2s']:
            token_type = KeyDetector.detect_nested_key(key, emitter, 2)
            assert token_type == TokenType.NESTED_KEY
    
    def test_ui_element_option_keys(self):
        """Test UI element option keys inside a UI element block"""
        emitter = make_emitter("zUI.zVaF.zolo")
        emitter.enter_block(BLOCK_ZIMAGE, 2, 0)
        
        # Keys in UI_ELEMENT_PROPERTY_KEYS get highlighted inside UI blocks
        for key in ['src', 'quality', 'alt_text']:
            token_type = KeyDetector.detect_nested_key(key, emitter, 4)
            assert token_type == TokenType.UI_ELEMENT_PROPERTY_KEY
        
        # Keys not in UI_ELEMENT_PROPERTY_KEYS stay regular nested keys
...
class TestBlockEntryDetection:
    ...
    def test_ui_element_block_entries(self):
        ...
        # Plural shorthands are no longer block entries via should_enter_block
        assert KeyDetector.should_enter_block('zURLs', emitter) is None
...
class TestHelperFunctions:
    ...
    def test_detect_key_type_root(self):
        ...
        # zSpark highlighting is parked (being rebuilt), so ROOT_KEY is expected.
        emitter = make_emitter("zSpark.example.zolo")
...
class TestKeyDetectorSets:
    """Test key set constants (moved from KeyDetector to token_registry)"""
    
    def test_zos_data_keys_complete(self):
        """Test ZOS_DATA_KEYS set completeness"""
        from zlsp.token_registry import ZOS_DATA_KEYS
        required = {'Data_Type', 'Data_Label', 'Data_Source', 'Schema_Name', 'zMigration', 'zMigrationVersion'}
        assert required.issubset(ZOS_DATA_KEYS)
    
    def test_zschema_property_keys_complete(self):
        """Test ZSCHEMA_PROPERTY_KEYS set completeness"""
        from zlsp.token_registry import ZSCHEMA_PROPERTY_KEYS
        required_keys = {'type', 'pk', 'required', 'default', 'rules'}
        assert required_keys.issubset(ZSCHEMA_PROPERTY_KEYS)
    
    def test_ui_element_keys_complete(self):
        """Test UI_ELEMENT_KEYS set completeness"""
        from zlsp.token_registry import UI_ELEMENT_KEYS
        required_keys = {'zImage', 'zText', 'zMD', 'zURL', 'zH1', 'zH2', 'zH3', 'zH4', 'zH5', 'zH6'}
        assert required_keys.issubset(UI_ELEMENT_KEYS)
